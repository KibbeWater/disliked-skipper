...
export function fetchRatings(songs: string[]): Promise<Record<string, Rating>> {
  const uncached = _filterUncached(songs);
  const cached = songs.filter((song) => !uncached.includes(song));
  if (uncached.length === 0)
    return Promise.resolve(
      Object.fromEntries(
        cached.map((song) => [song, ratings_cache[song].value]),
      ),
    );
    ...
    _fetchRatings(uncached)
      .then((ratings) => {
        ...
        const missingRatings = uncached
          .filter(
            (song) => !fetchedRatings.some((rating) => rating.song === song),
          )
...
const _filterUncached = (songs: string[]) =>
  songs.filter(
    (song) => !ratings_cache[song] || !isCachedItemValid(ratings_cache[song]),
  );
const _storeRatings = (ratings: Rating[]) =>
  ratings.forEach(
    (rating) =>
      (ratings_cache[rating.song] = { value: rating, cache_date: Date.now() }),
  );

// const CACHE_LIFETIME = 1000 * 60 * 60 * 1; // 1 hour
const isCachedItemValid = (item: Cached<Rating>) =>
  Date.now() - item.cache_date < 1000 * 60 * useConfig().cacheDuration &&
  useConfig().enableCache;
...
async function _fetchRatings(songs: string[]) {
  const ratings = await _musicAPIRequest<{ data: _rating[] }>(
    `/v1/me/ratings/songs?ids=${songs.join(",")}`,
  );
  return ratings.data;
}
